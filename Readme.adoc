...
You can also create a standalone executable Jar file of the project by using the `package` goal. The resulting Jar file created in the `target` directory can simply be run as any executable, e.g. `./target/demo.jar` or by running it with `java -jar ...`.
